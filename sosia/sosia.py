...
class Original(Scientist):
    ...
    def define_search_group(self, stacked=False, verbose=False, refresh=False):
        ...
        sources_ys = (pd.DataFrame(list(product(search_sources,
                           _years_search)), columns=['source_id', 'year']))
        ...
        if stacked:
            ...
            today = set([au for l in
                        sources_ys[sources_ys.year==self.year].
                            auids.tolist()
                        for au in l])
            then = set([au for l in
                        sources_ys[sources_ys.year.
                                      between(_min_year, _max_year,
                                              inclusive=True)].
                            auids.tolist()
                        for au in l])
            negative = set([au for l in
                        sources_ys[sources_ys.year<_min_year].
                            auids.tolist()
                        for au in l])
        else:
            today = set()
    ...
    def find_matches(self, stacked=False, verbose=False, stop_words=STOPWORDS,
        ...
        if authors_search:
            params = {"group": authors_search, "res": [],
            ...
            res = res[['auth_id', 'eid', 'surname', 'initials', 'givenname',
                       'affiliation', 'documents','affiliation_id', 'city',
                       'country', 'areas']]
            cache_authors(res)
        authors_cache, _ = authors_in_cache(authors)
        group = authors_cache[(authors_cache.areas.str.startswith(self.main_field[1])) &
                    (authors_cache.documents.astype(int) >= int(min(_npapers)))]['auth_id'].tolist()
        group.sort()
        n = len(group)
        ...
        if stacked:  # Combine searches
            if not author_year_search.empty:
                ...
                params = {"group": auth_year_group,
                          "res": [], "query": q, "refresh": refresh,
            ...
            matches = (author_year_cache[(author_year_cache.first_year.
                       between(min(_years),max(_years)))&(author_year_cache.
                       n_pubs.between(min(_npapers),max(_npapers)))&
                       (author_year_cache.n_coauth.between(min(_ncoauth)
                       ,max(_ncoauth)))]['auth_id'].tolist())
